...
flowchart TD
    req["POST /users + JSON"]
    req --> router["routers/users.py —— 接口层<br/>接收请求，声明参数和响应模型"]
    router -->|"用 schemas.UserCreate 校验请求体<br/>通过 Depends(get_db) 拿到数据库会话"| crud["crud.py —— 数据访问层<br/>create_user(db, user_in)"]
    crud -->|"把 Pydantic 模型转成 SQLAlchemy 模型对象"| model["models.py + database.py —— 数据层<br/>User 对象经 Session 写入数据库"]
    model --> resp["返回 User 对象 → 按 schemas.UserOut 序列化成 JSON → 响应"]
```
...
下一章：[4.2 数据库会话与依赖注入](02-数据库会话与依赖注入.md) —— 写 database.py 和 models.py。
