...
```mermaid
flowchart TD
    push[git push] -->|Webhook| jenkins["Jenkins（node:20 容器里构建）"]
    jenkins --> install[pnpm install]
    install --> build["pnpm build → 产出 dist/"]
    build --> deploy["通过 SSH 把 dist/ 同步到 Web 服务器的 Nginx 目录"]
```
